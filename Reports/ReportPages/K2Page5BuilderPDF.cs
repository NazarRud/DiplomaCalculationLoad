...
{
    public class K2Page5BuilderPDF : PDFPageBuilder
    {
        public override void CreateHeader()
        {
            ...
            row.Shading.Color = Colors.Wheat;
            cell = row.Cells[0];
            cell.AddParagraph("Аудиторні заняття").Format.Alignment = ParagraphAlignment.Center;
            cell.Format.Font.Bold = true;
            cell.MergeRight = 11;
            cell = row.Cells[12];
            cell.AddParagraph("Екзамени").Format.Alignment = ParagraphAlignment.Center;
            cell.MergeRight = 3;
            cell.MergeDown = 1;
            ...
            cell.MergeRight = 3;
            cell = row.Cells[4];
            cell.AddParagraph("Практичні\nзаняття\n(семінари)").Format.Alignment = ParagraphAlignment.Center;
            cell.MergeRight = 3;
            cell = row.Cells[8];
            cell.AddParagraph("Лаборатор,заняття\n(Комп'ютерн. практ)").Format.Alignment = ParagraphAlignment.Center;
            cell.MergeRight = 3;
            ...
            for (int i = 0; i < 8; i++)
            {
                currentCell += 2;
                cell = row.Cells[currentCell];
                cell.AddParagraph((i + 17).ToString()).Format.Alignment = ParagraphAlignment.Center;
                cell.MergeRight = 1;
            }
